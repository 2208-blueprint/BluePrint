...
const syncAndSeed = async () => {
  try {
    ...
    console.log("should have three followers", subscriberKing.followers.length);
    ben.removeFollowing(thomas);
    const afterRemoving = await User.findByPk(1, {
      include: [{ model: User, as: "followers", attributes: ["username"] }],
    });
    console.log("should have two followers", afterRemoving.followers.length);
